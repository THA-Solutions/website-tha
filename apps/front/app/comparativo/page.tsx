...
export default function Page() {
  ...
  useEffect(() => {
    const getInverters = async () => {
      setInverters(await InverterService.getAllInverters());
    };
        ...
        <>
          <main className="bg-backgroundAlt2 ring-1 ring-gray-700 mt-8 flex flex-col items-center justify-between p-8 gap-6 md:flex-row">
            ...
            <Icon
              ...
              className="hidden text-6xl text-gray-600 md:flex"
            />
            <Icon
              icon="mdi:compare-vertical"
              className="text-6xl text-gray-600 md:hidden"
            />
            ...
            {inverterData !== null && inverterData2 !== null && (
              <Link
                href={`/comparativo/${inverterData.id}/${inverterData2.id}`}
                className="p-2 flex items-center justify-center gap-2 w-full text-center text-background uppercase font-semibold transition-all bg-tertiary sm:w-80 hover:bg-tertiary/70"
              >
                <span>Comparar</span>
                <ArrowRightAlt />
              </Link>
            ...
            {inverters.map((inverter: Inverter, index: number) => (
              ...
              >
                ...
                <div className="flex flex-col gap-2">
                  ...
                  {inverterData?.id !== inverter.id &&
                  inverterData2?.id !== inverter.id ? (
                    <button
